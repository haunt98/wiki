...
- [Random notes on improving the Redis LRU algorithm](https://antirez.com/news/109)
- [Redis streams as a pure data structure](https://antirez.com/news/128)
...
- [Upgrading Uber’s MySQL Fleet to version 8.0](https://www.uber.com/us/en/blog/upgrading-ubers-mysql-fleet/)
...
- [MySQL 8.0 Hot Rows with NOWAIT and SKIP LOCKED](https://www.percona.com/blog/mysql8-hot-rows-with-nowait-skip-locked/)
...
> SKIP LOCKED is what makes this scalable: if another transaction has locked some rows, MySQL skips them and returns
> other available rows. No waiting on the same row, less contention.
>
> Our first prototype used an auto-increment ID as the primary key. When we observed lock behavior (e.g. with SHOW
> ENGINE INNODB STATUS), we saw two row locks per reservation instead of one. With an auto-increment primary key, InnoDB
> was locking both the secondary index used in the WHERE clause and the clustered index (primary key). We switched to a
> composite primary key (shop_id, inventory_item_id, inventory_group_id, id) so the columns we filter on are part of the
> primary key. That reduced to one lock per row, which was important when running many reservations per second.
>
> When we ran SELECT ... FOR UPDATE SKIP LOCKED on an empty table that needed replenishment, we saw gap locks (including
> on the "supremum" pseudo-record). Those locks blocked the replenishment transaction from inserting new rows and could
> lead to deadlocks. We changed the transaction isolation level from REPEATABLE READ (MySQL default) to READ COMMITTED
> for these transactions. Under READ COMMITTED, InnoDB doesn't take gap locks in the same way, so replenishment could
> proceed.
...
- [Postgres’ Original Project Goals: The Creators Totally Nailed It](https://www.crunchydata.com/blog/the-postgres-project-original-goals-and-how-the-creators-totally-nailed-it)
...
- [The Log Data Structure](https://topicpartition.io/definitions/the-log)
...
>
> HDD-friendly: it’s incredibly efficient for hard drives because of its sequential read/write patterns. This aligns
> with how HDDs perform best — large, linear IO operations through its mechanical actuator arm and spinning platter
> design. It benefits from OS-level and hardware-level IO batching.
>
> big data friendly: its performance doesn’t degrade with size. A 20TB log performs roughly the same as a 1GB log.
>
> read parallelism friendly: because it’s append-only, there is no need for locking when multiple readers are accessing
> it.
>
...
- message type:
    ...
    - queuing: classical queuing use cases
    - tracking: tracking user clicks, ...
    - streaming
    - push: push from offline to online environments
    - user
...
- Need to consume topic exactly, lacking topic hiearrarchy
- Need schema registry, if push malicious data can panic consumer
- Need topic management in one place, because topic name is shared between many systems
- Consumer Group is a way to scale consumer, all services in same Consumer Group will share consuming messages without
  duplicating
...
- [Managing Kafka topic configuration](https://cnr.sh/posts/2019-02-04-managing-kafka-topic-configuration/)
...
>
> They spread the data out in many (many!) hard drives so they can achieve massive read throughput by utilizing each
> drive in parallel.
>
>     Storing a 1 TB file in a single HDD means limits your reading rate by that single drive’s max throughput (~300 MB/s).
>
...
>
> S3 uses Erasure Coding (EC). It breaks data into K shards with M redundant “parity” shards. EC allows you to
> reconstruct the data from any K shards out of the total K+M shards.
>
...
>
> Instead of sending one request through a single thread and HTTP connection, the client chunks it into 10 parts and
> uploads each in parallel.19
>
...
>
> Uploading 1 GB/s to a single server may be difficult, but uploading 100 chunks each at 10 MB/s chunks to 100 different
> servers is very practical.
...
- [DSQL: Simplifying Architectures](https://brooker.co.za/blog/2025/11/02/thinking-dsql.html)
...
- [DSQL Vignette: Transactions and Durability](https://brooker.co.za/blog/2024/12/05/inside-dsql-writes.html)
- [DSQL Vignette: Wait! Isn’t That Impossible?](https://brooker.co.za/blog/2024/12/06/inside-dsql-cap.html)
- [Decomposing Aurora DSQL](https://brooker.co.za/blog/2025/04/17/decomposing.html)
- [Aurora DSQL: How authentication and authorization works](https://marc-bowes.com/dsql-auth.html)
- [Aurora DSQL and the Circle of Life](https://marc-bowes.com/dsql-circle-of-life.html)
